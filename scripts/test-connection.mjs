...
async function testAllDatabases() {
  ...
  try {
    const places = await notion.databases.query({
      database_id: process.env.NOTION_PLACES_DB,
      page_size: 1,
    });
  ...
  console.log('\n📧 NEWSLETTER');
  try {
    const newsletter = await notion.databases.query({
      database_id: process.env.NOTION_NEWSLETTER_DB,
      page_size: 1,
    });
  ...
  console.log('\n🗺️ GUIDES');
  try {
    const guides = await notion.databases.query({
      database_id: process.env.NOTION_GUIDES_DB,
      page_size: 1,
    });
